...
async function getHandler(request: AuthenticatedRequest) {
  try {
    ...
    if (token && request.user) {
      console.log("[API Student] Using user-scoped client (Bearer token)");
      const { getDatabaseClientAsUser } = await import(
        "@/app/shared/core/database/database"
      );
      client = getDatabaseClientAsUser(token);
    } else if (request.apiKey) {
      console.log("[API Student] Using service role client (API Key)");
      client = getServiceRoleClient();
    } else {
      console.log("[API Student] Using cookie-based client (or unauthenticated)");
