...
- [ ] **STRP-01**: Webhook handler é idempotente (tabela `webhook_events` com `stripe_event_id` UNIQUE, deduplicação via INSERT ON CONFLICT)
- [ ] **STRP-02**: Webhook handler usa padrão single-sync (`syncStripeSubscriptionToLocal()`) — busca estado atual do Stripe em vez de aplicar updates incrementais
...
- [ ] **STRP-05**: Logging estruturado substitui todos os `console.log` no webhook handler e rotas de billing
...
- [ ] **RESIL-01**: Eventos de webhook são persistidos em tabela `webhook_events` com status (received, processed, failed)
